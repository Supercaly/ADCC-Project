...
Una volta clonata la repository localmente è necessario compilare tutti i file .erl eseguendo i seguenti comandi nel terminale:
...
**ts_manager.erl** è un ulteriore componente fondamentale dell'applicazione, si occupa della gestione delle letture e scritture all'interno di un TS; ogni spazio di tuple a cui ha accesso il nodo locale, ha un corrispettivo processo ts_manager in esecuzione e registrato con lo stesso nome.
...
Come già detto in precedenza, main_supervisor è il processo padre di tutta l'applicazione. Si è scelto di utilizzare un supervisor a questo livello per poter sfruttare a pieno il pattern OTP e garantire la costante esecuzione dei processi figli in ogni momento, anche in caso di crash; il supervisor, infatti, essendo di tipo *one_for_one*, monitora ogni processo e lo riavvia autonomamente in caso di terminazione improvvisa.
Anche ts_supervisor è stato creato con il medesimo scopo, ovvero garantire la costante esecuzione dei processi figli; in questo caso però, avendo a che fare con un numero di processi dinamico, abbiamo scelto di usare il *simple_one_for_one* supervisor.
...
In questa sezione analizzeremo alcune statistiche ricavate eseguendo dell'applicazione in un cluster composto da diversi nodi.

Per facilitare il monitoraggio di queste tempistiche è stato scritto un *escript* ovvero programma erlang che può essere lanciato come un comune script; per eseguirlo basta lanciare il seguente comando da terminale:
